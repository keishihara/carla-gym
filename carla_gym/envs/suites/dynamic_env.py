...
class DynamicCarlaEnv(CarlaEnv):
    def __init__(
        ...
        all_tasks = self.build_all_tasks(num_npc_vehicles, num_npc_walkers, weather_group, map_name)
    ...
    @staticmethod
    def build_all_tasks(
        num_npc_vehicles: int | list[int] | None,
        num_npc_walkers: int | list[int] | None,
        weather_group: str,
        map_name: str,
    ) -> dict:
        ...
        if num_npc_vehicles is None:
            num_npc_vehicles = [0, DEFAULT_NUM_NPC_VEHICLES[map_name]]
        if num_npc_walkers is None:
            num_npc_walkers = [0, DEFAULT_NUM_NPC_WALKERS[map_name]]
    ...
    def get_current_npc_counts(self) -> dict:
        ...
        if hasattr(self, "_task") and self._task is not None:
            return {
                "npc_vehicles": self._task.get("num_npc_vehicles", 0),
                "npc_walkers": self._task.get("num_npc_walkers", 0),
            }
