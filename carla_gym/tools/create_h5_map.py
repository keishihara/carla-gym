...
from carla_gym.utils.port import find_random_free_port
...
def kill_all_carla_servers(ports):
    ...
    for proc in psutil.process_iter():
        ...
        try:
            proc_connections = proc.net_connections(kind="all")
...
class MapImage:
    ...
    def draw_map_image(carla_map_local, pixels_per_meter_local, precision=0.05):
        ...
        for waypoint in tqdm(topology, dynamic_ncols=True):
            waypoints = [waypoint]
...
def main():
    ...
    parser.add_argument("--town", required=True, type=str, choices=MAP_NAMES, help="town to generate the map for")
    parser.add_argument("--save_dir", default="maps_{ppm}ppm")
    parser.add_argument("--pixels_per_meter", type=float, default=5.0)
    parser.add_argument("--carla_sh_path", default="/data/workspace/carla_0_9_15/CarlaUE4.sh")
    parser.add_argument("--overwrite", action="store_true", help="overwrite existing map")
    args = parser.parse_args()

    print(f"Generating map for {args.town} with pixels_per_meter={args.pixels_per_meter:.2f}.")

    save_dir = Path(args.save_dir.format(ppm=args.pixels_per_meter))
    save_dir.mkdir(parents=True, exist_ok=True)
    print(f"Saving {args.town} map to {save_dir}")

    h5_file_path = save_dir / (args.town + ".h5")
    if not args.overwrite and is_h5_map_exists(h5_file_path, args.pixels_per_meter):
        print(f"{str(h5_file_path.name)} with pixels_per_meter={args.pixels_per_meter:.2f} already exists. Skipping...")
        return

    current_port_0 = find_random_free_port()
    print(f"Using port {current_port_0} for the carla server.")

    client_ports, carla_servers = [], []
    client_ports.append(current_port_0)
    carla_servers.append(
        subprocess.Popen(
            f"bash {args.carla_sh_path} -nosound -RenderOffScreen -carla-rpc-port={current_port_0} -carla-streaming-port=0 -nullrhi",
            shell=True,
        )
    )
    time.sleep(60)
    ...
    world = client.load_world(args.town, reset_settings=False)
    dict_masks = MapImage.draw_map_image(world.get_map(), pixels_per_meter)

    print(f"Writing {h5_file_path.name} with pixels_per_meter={pixels_per_meter:.2f}.")
