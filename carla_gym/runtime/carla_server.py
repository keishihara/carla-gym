...
HOST_CARLA_SCRIPT = (
    f"/data/workspace/carla_0_9_15/CarlaUE4.sh:/home/{getpass.getuser()}/workspace/carla_0_9_15/CarlaUE4.sh:"
)
...
class CarlaServerManager:
    ...
    def start(
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                device = "CPU" if self.nullrhi else f"GPU {self.gpu_index}"
                logger.info(f"CARLA server ({device}) ready on port {self.rpc_port}")
                return CarlaPorts(
    ...
    def _build_command(self) -> list[str]:
        ...
        if using_apptainer:
            script = self.container_script
        else:
            # Find the script in the host machine
            for path in self.host_script.split(":"):
                if Path(path).exists():
                    script = path
                    break
            else:
                raise FileNotFoundError(f"CARLA script not found in {self.host_script}")
    ...
    def _should_use_apptainer(self) -> bool:
        ...
        if self.use_apptainer == "auto":
            ...
            # NOTE: Now we have Carla 0.9.15 running on the host machine of H100 GPUs
            # So we no longer need to use Apptainer, but we keep this just in case
            return False
