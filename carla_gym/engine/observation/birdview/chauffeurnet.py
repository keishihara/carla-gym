...
class Birdview(BaseObservation):
        ...
        self.obs_space = spaces.Dict(
            {
                ...
                "masks": spaces.Box(
                    low=0, high=255, shape=(self._masks_channels, self._width, self._width), dtype=np.int64
                ),
            }
        )

    def attach_ego_vehicle(self, parent_actor: EgoVehicle):
    ...
    def get_observation(self):
        ...
        # road_mask, lane_mask
        road_mask = warp_affine(self._road, M_warp, (self._width, self._width)).astype(bool)
        lane_mask_all = warp_affine(self._lane_marking_all, M_warp, (self._width, self._width)).astype(bool)
        lane_mask_broken = warp_affine(self._lane_marking_white_broken, M_warp, (self._width, self._width)).astype(bool)
    ...
    def _get_mask_from_stopline_vtx(self, stopline_vtx, M_warp):
        ...
        for sp_locs in stopline_vtx:
            ...
            pt1 = np.round(stopline_warped[0, 0]).astype(np.int32)
            pt2 = np.round(stopline_warped[1, 0]).astype(np.int32)
